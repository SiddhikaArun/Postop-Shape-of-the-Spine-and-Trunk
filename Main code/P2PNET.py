...
Model = collections.namedtuple("Model", \
                               ...
                               train_dataloss_A_ph,  train_dataloss_B_ph, train_regul_ph, train_rigidloss_A_ph, train_rigidloss_B_ph\
                               test_dataloss_A_ph,   test_dataloss_B_ph,  test_regul_ph, test_rigidloss_A_ph, test_rigidloss_B_ph"                    )
...
def create_model( FLAGS  ):
    ...
    learning_rate = tf.train.exponential_decay(
        0.001,  # base learning rate 0.001
    ...
    #     pointSet_meta_A_ph = tf.placeholder_with_default(Train_examples_shuffled.pointsets_meta_A, shape=(FLAGS.batch_size, FLAGS.point_num, 6))
    #     pointSet_T_A_ph = tf.placeholder_with_default(Train_examples.pointsets_T_A, shape=(FLAGS.batch_size, FLAGS.vertebra_num, 3))
    #     pointSet_R_A_ph = tf.placeholder_with_default(Train_examples.pointsets_R_A, shape=(FLAGS.batch_size, FLAGS.vertebra_num, 3))
    #     pointSet_T_B_ph = tf.placeholder_with_default(Train_examples.pointsets_T_B, shape=(FLAGS.batch_size, FLAGS.vertebra_num, 3))
    #     pointSet_R_B_ph = tf.placeholder_with_default(Train_examples.pointsets_R_B, shape=(FLAGS.batch_size, FLAGS.vertebra_num, 3))
    # else:
    ...
    #     pointSet_meta_A_ph = tf.placeholder_with_default(Test_examples_shuffled.pointsets_meta_A, shape=(FLAGS.batch_size, FLAGS.point_num, 6))
    #     pointSet_T_A_ph = tf.placeholder_with_default(Test_examples.pointsets_T_A, shape=(FLAGS.batch_size, FLAGS.vertebra_num, 3))
    #     pointSet_R_A_ph = tf.placeholder_with_default(Test_examples.pointsets_R_A, shape=(FLAGS.batch_size, FLAGS.vertebra_num, 3))
    #     pointSet_T_B_ph = tf.placeholder_with_default(Test_examples.pointsets_T_B, shape=(FLAGS.batch_size, FLAGS.vertebra_num, 3))
    #     pointSet_R_B_ph = tf.placeholder_with_default(Test_examples.pointsets_R_B, shape=(FLAGS.batch_size, FLAGS.vertebra_num, 3))
    ...
    # pointSet_meta_A_ph = tf.placeholder( tf.float32, shape=(FLAGS.batch_size, FLAGS.point_num, 6) )
    ...
    with tf.variable_scope("p2pnet_A2B") as scope:
        # displace_A2B = get_displacements( pointSet_A_ph, is_training_ph, noise1, pointSet_meta_A_ph,  FLAGS, bn_decay  )
    ...
    with tf.variable_scope("p2pnet_B2A") as scope:
        # displace_B2A = get_displacements(c, is_training_ph, noise2, pointSet_meta_A_ph, FLAGS, bn_decay)
        # displace_B2A = get_displacements( pointSet_B_ph, is_training_ph, noise2, pointSet_meta_A_ph, FLAGS, bn_decay  )
    ...
    # Predicted_A = pointSet_A_ph + dis_A2B
    # Predicted_B = pointSet_B_ph + dis_B2A

    # Predicted_A = pointSet_A_ph + displace_A2B
    # Predicted_B = pointSet_B_ph + displace_B2A
    # # # # # # # # # #


    # Predicted_A = pointSet_B_ph + dis_B2A
    # Predicted_B = pointSet_A_ph + dis_A2B
    ...
    training_sum_ops = tf.summary.merge( \
        [lr_sum_op, train_dataloss_A_sum_op, train_dataloss_B_sum_op, train_regul_sum_op, train_rigidloss_A_sum_op, train_rigidloss_B_sum_op])

    testing_sum_ops = tf.summary.merge( \
        [test_dataloss_A_sum_op, test_dataloss_B_sum_op, test_regul_sum_op, test_rigidloss_A_sum_op, test_rigidloss_B_sum_op ])
    ...
    return Model(
        ...
        train_dataloss_A_ph=train_dataloss_A_ph, train_dataloss_B_ph=train_dataloss_B_ph, train_regul_ph=train_regul_ph, train_rigidloss_A_ph=train_rigidloss_A_ph, train_rigidloss_B_ph=train_rigidloss_B_ph,
        test_dataloss_A_ph=test_dataloss_A_ph, test_dataloss_B_ph=test_dataloss_B_ph, test_regul_ph=test_regul_ph, test_rigidloss_A_ph=test_rigidloss_A_ph, test_rigidloss_B_ph=test_rigidloss_B_ph
    )
...
def get_displacements(input_points, is_training, noise, FLAGS, bn_decay=None):
    ...
    # l1_xyz, l1_points, l1_indices = pointnet_sa_module(l0_xyz, l0_points, npoint=50, radius=0.1 * FLAGS.radiusScal,
    #                                                    nsample=20, #64
    #                                                    mlp=[20, 20, 40], mlp2=None, group_all=True,
    #                                                    is_training=is_training, bn_decay=bn_decay, scope='layer1')
    # l2_xyz, l2_points, l2_indices = pointnet_sa_module(l1_xyz, l1_points, npoint=25, radius=0.2 * FLAGS.radiusScal,
    #                                                    nsample=20,
    #                                                    mlp=[40, 40, 80], mlp2=None, group_all=True,
    #                                                    is_training=is_training, bn_decay=bn_decay, scope='layer2')
    # l3_xyz, l3_points, l3_indices = pointnet_sa_module(l2_xyz, l2_points, npoint=10, radius=0.4 * FLAGS.radiusScal,
    #                                                    nsample=20,
    #                                                    mlp=[80, 80, 160], mlp2=None, group_all=True,
    ...
    #                                                    mlp=[160, 160, 320], mlp2=None, group_all=True,
    ...
    # l3_points = pointnet_fp_module(l3_xyz, l4_xyz, l3_points, l4_points, [160, 160], is_training, bn_decay, scope='fa_layer1')
    # l2_points = pointnet_fp_module(l2_xyz, l3_xyz, l2_points, l3_points, [160, 80], is_training, bn_decay, scope='fa_layer2')
    # l1_points = pointnet_fp_module(l1_xyz, l2_xyz, l1_points, l2_points, [80, 40], is_training, bn_decay, scope='fa_layer3')
    # l0_points = pointnet_fp_module(l0_xyz, l1_xyz, l0_points, l1_points, [40, 40, 40], is_training, bn_decay, scope='fa_layer4')
    ...
    #
    # net = tf_util.conv1d(l0_points, 40, 1, padding='VALID'  , bn=True, is_training=is_training, scope='fc1', bn_decay=bn_decay )
    # net = tf_util.conv1d(net, 20, 1, padding='VALID', bn=True, is_training=is_training, scope='fc2', bn_decay=bn_decay)
    # net = tf_util.conv1d(net, 3, 1, padding='VALID', activation_fn=None, scope='fc3')
...
def get_Regularizing_Loss(pointSet_A_ph, pointSet_B_ph,  Predicted_A, Predicted_B):
    ...
    # displacements_A = pairwise_l2_norm2_batch (pointSet_A_ph, Predicted_B)
    # displacements_B = pairwise_l2_norm2_batch (Predicted_A,   pointSet_B_ph)
...
def rigid_transform_3D(Predicted_shape, target_T, target_R, FLAGS, scope=None):
    #with tf.op_scope([Predicted_shape], scope, 'rigid_transform_3D'): # A: shape(102,3)
       ...
       ### A_array_all = np.array(Predicted_shape)
         ...
         # translations = tf.zeros((batch_size, 16, 3), dtype=np.float32) #translations = np.zeros((batch_size, 16, 3), dtype=np.float32)
         # rotations = tf.zeros((batch_size, 16, 3), dtype=np.float32)
         ...
         #   c = tf.zeros((17, 3), dtype=np.float32) #c = np.zeros((17, 3))
         #   vecZ = tf.zeros((3), dtype=np.float32) #vecZ = np.zeros(3)
         #   vecZ = tf.zeros((3), dtype=np.float32) #vecY = np.zeros(3)
         #   vecZ = tf.zeros((3), dtype=np.float32) #vecX = np.zeros(3)
         #   vec = tf.zeros((batch_size, 16, 3), dtype=np.float32) #vec = np.zeros((17, 3, 3))
         #   trans = tf.zeros((16, 3), dtype=np.float32) #trans = np.zeros((16, 3), dtype=np.float32)
         #   rotate = tf.zeros((16, 3), dtype=np.float32) #rotate = np.zeros((16, 3), dtype=np.float32)
         ...
         #square_dist = pairwise_l2_norm2_batch(target_T, translations_t)
         ...
         #
         # #square_dist2 = pairwise_l2_norm2_batch(target_R, rotations_t)
         #
         #
         # # rotations_reshape = tf.reshape(rotations[:, :, :, :], [3, 16, 9])
         # # target_R_reshape = tf.reshape(target_R[:, :, :, :], [3, 16, 9])
         #
         # #square_dist2 = pairwise_l2_norm2_batch_high_dim(target_R, rotations)
