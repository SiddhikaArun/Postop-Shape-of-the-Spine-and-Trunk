...
for i=1:length(files)
    sample = files(i).name;
    source_pre = ['.\' sample '\T_R_ply\' sample '.pre_T.ply'];
    source_post = ['.\' sample '\T_R_ply\' sample '.post_T.ply'];
    
    dest_pre = ['.\' sample '\T_R_ply\' sample '.pre_new_T.ply'];
    dest_post = ['.\' sample '\T_R_ply\' sample '.post_new_T.ply'];
...
%     
    %data_pre_new = pointCloud([data_pre.Location(:,1),data_pre.Location(:,2),z_pre]);
    data_pre_new = pointCloud([x_pre,y_pre,z_pre]);
    pcwrite(data_pre_new, dest_pre)
    
    %data_post_new = pointCloud([data_post.Location(:,1),data_post.Location(:,2),z_post]);
