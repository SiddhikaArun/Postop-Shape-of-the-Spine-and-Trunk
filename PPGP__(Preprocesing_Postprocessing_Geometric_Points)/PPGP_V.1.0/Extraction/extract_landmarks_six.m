...
%           Num3D_ = axesvertebrescalc(Num3D);
%           [center_curve vect]=axesextract(Num3D_);
%           center_curve=center_curve(:,7:23)';
% 
%           curve=fnplt(cscvn(center_curve(:,:)'));
%           curve=curve';
%           length_curve=arclength(curve(:,1), curve(:,2), curve(:,3));
%           Landmarks_array_norm=Landmarks_array/length_curve
% 
%           pcwrite(pointCloud(Landmarks_array_norm), dest);
